...
var updateRequest = function(params,callback){
   var urlObj = require('url').parse(params.fullPath);
   urlObj.protocol = 'http:';
   urlObj.hostname = params.host;
   urlObj.port = params.port;

   var url = require('url').format(urlObj);
   var requestOptions = {
      url: url,
      json: params.json
   };
   ...
   if(params.agent !== undefined){
      if(params.agent === false) {
         requestOptions.pool = false;
      } else {
         requestOptions.agent = params.agent;
      }
   }
   ...
   request(requestOptions, function(err, resp, data) {
     if(callback) {
        if(err) {
           ...
           callback(new SolrError(resp.statusCode,data),null);
        } else {
           callback(err,data);
        }
     }
